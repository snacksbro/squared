...
class Square extends React.Component {
	...
	sendRequest() {
		http
			.get("http://127.0.0.1:5000/verify?square=" + this.props.squareID)
			.then(function (res) {
				//alert(res.data);
			});
		/* Old implementation
	...
	render() {
		...
		return (
			<td
				className={this.props.claim + "-captured board-square"}
				style={{ backgroundImage: `url(${this.state.icon})` }}
				id={this.props.squareID}
				onClick={this.sendRequest}></td>
		);
	}
}
	...
	render() {
		...
		for (let i = 0; i <= 10; i++) {
			row.push(
				<Square
					claim={
						this.props.boardData.board?.[i][
							alphabet.indexOf(this.props.rowChar)
						]
					}
					boardData={this.props.boardData}
					squareID={i + this.props.rowChar}
				/>,
			);
		}
	...
	render() {
		...
		for (let i = 0; i < boardCharacters.length; i++) {
			if (this.props.game != undefined) {
				boardObj.push(
					<BoardRow boardData={this.props.game} rowChar={boardCharacters[i]} />,
				);
			}
		}
